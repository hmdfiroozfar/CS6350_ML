...
Original file is located at
    https://colab.research.google.com/drive/1-6p3vuzhbIJnwrZMc2enjFezGJKC9CgP
"""
...
def bagging_error(forest, data, labels):
...
def main(num_trees, depth):
    processed_train, processed_test = process_data('Data/Bank/train.csv', 'Data/Bank/test.csv', COLUMN_DICT, COLUMN_NAMES)
    ...
    for _ in range(num_iters):
        ...
        bagged_trees.append(DecisionTree(sample_data, sample_labels, attributes=[i for i in range(len(sample_data[0]))], depth=depth))

    train_errors = bagging_error(bagged_trees, training_data, training_labels)
    test_errors = bagging_error(bagged_trees, testing_data, testing_labels)

    plt.plot(train_errors, color='blue', label="Train")
    plt.plot(test_errors, color='red', label="Test")
    plt.title("Bagged Trees Error vs Number of Trees", color='black')
    plt.legend()
    plt.show()
